...
# Hugeicons SVG size options
HUGEICONS_SIZES = [16, 20, 24, 28, 32, 36, 40, 48, 56, 64, 72, 80, 96, 128]
...
def build_hugeicons_size_keyboard(token: str) -> InlineKeyboardMarkup:
    ...
    for size in HUGEICONS_SIZES:
        row.append(InlineKeyboardButton(
            str(size), callback_data=f"hisz|{size}|{token}"
        ))
        if len(row) == 4:
            rows.append(row)
            row = []
    ...
    rows.append([
        InlineKeyboardButton(
            "🔺 SVG file (all sizes)", callback_data=f"hisz|svg|{token}"
        )
    ])
...
def build_lummi_size_keyboard(token: str) -> InlineKeyboardMarkup:
    rows = []
    for key, label in LUMMI_SIZE_LABELS.items():
        rows.append([
            InlineKeyboardButton(label, callback_data=f"lmsz|{key}|{token}")
        ])
    rows.append([
        InlineKeyboardButton("📦 Original file", callback_data=f"lmsz|orig|{token}")
    ])
    return InlineKeyboardMarkup(rows)
...
def build_effect_toolbox_keyboard(token: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "🎨 Pick an Effect", callback_data=f"fxtool|effect|{token}"
            )
        ],
        [
            InlineKeyboardButton(
                "📐 Aspect Ratio", callback_data=f"fxtool|aspect|{token}"
            ),
            InlineKeyboardButton(
                "📏 Resize", callback_data=f"fxtool|resize|{token}"
            ),
        ],
        [
            InlineKeyboardButton(
                "🎛 Customize Parameters", callback_data=f"fxtool|params|{token}"
            )
        ],
        [
            InlineKeyboardButton(
                "📋 Copy (as-is)", callback_data=f"fxtool|copy|{token}"
            ),
            InlineKeyboardButton(
                "🔁 Convert Format", callback_data=f"fxtool|convert|{token}"
            ),
        ],
        [
            InlineKeyboardButton(
                "📉 Compress", callback_data=f"fxtool|compress|{token}"
            ),
            InlineKeyboardButton(
                "🚫 Remove Watermark", callback_data=f"fxtool|wmremove|{token}"
            ),
        ],
        [
            InlineKeyboardButton(
                "✅ Apply & Continue", callback_data=f"fxtool|go|{token}"
            )
        ],
    ])
...
def build_effect_keyboard(category: str, token: str) -> InlineKeyboardMarkup:
    ...
    for key, label, cat in EFFECTS:
        if cat != category:
            continue
        row.append(InlineKeyboardButton(
            label, callback_data=f"fx|{key}|{token}"
        ))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
        buttons.append(row)
    buttons.append([
        InlineKeyboardButton("◀ Categories", callback_data=f"fxback|{token}")
    ])
...
def build_aspect_ratio_keyboard(token: str) -> InlineKeyboardMarkup:
    ...
    for key, label in ASPECT_RATIO_PRESETS:
        row.append(InlineKeyboardButton(
            label, callback_data=f"fxaspect|{key}|{token}"
        ))
        if len(row) == 2:
            rows.append(row)
            row = []
    ...
    rows.append([
        InlineKeyboardButton(
            "✂ Crop mode", callback_data=f"fxcropmode|crop|{token}"
        ),
        InlineKeyboardButton(
            "🖼 Pad mode", callback_data=f"fxcropmode|pad|{token}"
        ),
    ])
    rows.append([
...
def build_resize_presets_keyboard(token: str) -> InlineKeyboardMarkup:
    ...
    rows.append([
        InlineKeyboardButton(
            "Small (480px)", callback_data=f"fxresize|480|{token}"
        )
    ])
    rows.append([
        InlineKeyboardButton(
            "Medium (720px)", callback_data=f"fxresize|720|{token}"
        )
    ])
    rows.append([
        InlineKeyboardButton(
            "Large (1080px)", callback_data=f"fxresize|1080|{token}"
        )
    ])
    rows.append([
        InlineKeyboardButton(
            "XL (1600px)", callback_data=f"fxresize|1600|{token}"
        )
    ])
    rows.append([
        InlineKeyboardButton(
            "✏️ Custom WxH (type it)",
            callback_data=f"fxresizecustom|{token}",
        )
    ])
    rows.append([
        InlineKeyboardButton("◀ Back", callback_data=f"fxtoolback|{token}")
...
def build_param_keyboard(token: str, params: dict) -> InlineKeyboardMarkup:
    def row(label, key, step, fmt="{:.1f}"):
        ...
        return [
            ...
            InlineKeyboardButton(
                "➖", callback_data=f"fxparam|{key}|-{step}|{token}"
            ),
            InlineKeyboardButton(
                "➕", callback_data=f"fxparam|{key}|{step}|{token}"
            ),
    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton(
                "♻ Reset to defaults",
                callback_data=f"fxparamreset|{token}",
            )
        ],
        [
            InlineKeyboardButton(
                "◀ Toolbox", callback_data=f"fxtoolback|{token}"
            )
        ],
    ])
...
def build_translate_lang_keyboard() -> InlineKeyboardMarkup:
    ...
    for code, label in TRANSLATE_LANGUAGES:
        row.append(InlineKeyboardButton(
            label, callback_data=f"trlang|{code}"
        ))
        if len(row) == 2:
            buttons.append(row)
            row = []
...
def build_watermark_position_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "↖ Top-Left", callback_data="wmpos|top-left"
            ),
            InlineKeyboardButton(
                "↗ Top-Right", callback_data="wmpos|top-right"
            ),
        ],
        [
            InlineKeyboardButton(
                "↙ Bottom-Left", callback_data="wmpos|bottom-left"
            ),
            InlineKeyboardButton(
                "↘ Bottom-Right", callback_data="wmpos|bottom-right"
            ),
        ],
        [
            InlineKeyboardButton(
                "⏺ Centre", callback_data="wmpos|center"
            )
        ],
    ])
...
def build_limit_request_keyboard(
    chat_id: int, requested: int
) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[
        InlineKeyboardButton(
            "✅ Approve",
            callback_data=f"limitreq|approve|{chat_id}|{requested}",
        ),
        InlineKeyboardButton(
            "❌ Deny",
            callback_data=f"limitreq|deny|{chat_id}",
        ),
    ]])
...
def build_case_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "camelCase", callback_data="case|camel"
            ),
            InlineKeyboardButton(
                "PascalCase", callback_data="case|pascal"
            ),
        ],
        [
            InlineKeyboardButton(
                "snake_case", callback_data="case|snake"
            ),
            InlineKeyboardButton(
                "kebab-case", callback_data="case|kebab"
            ),
        ],
        [
            InlineKeyboardButton(
                "UPPERCASE", callback_data="case|upper"
            ),
            InlineKeyboardButton(
                "lowercase", callback_data="case|lower"
            ),
        ],
...
def build_media_format_keyboard(token: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(
                "🎬 Video (best, ≤720p)",
                callback_data=f"mediadl|video720|{token}",
            )
        ],
        [
            InlineKeyboardButton(
                "🎬 Video (best, ≤1080p)",
                callback_data=f"mediadl|video1080|{token}",
            )
        ],
        [
            InlineKeyboardButton(
                "🎵 Audio only (MP3)",
                callback_data=f"mediadl|audio|{token}",
            )
        ],
    ])
